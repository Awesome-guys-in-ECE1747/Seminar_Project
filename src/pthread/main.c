...
#define IPRINT(...)                                      \
	do                                                   \
	{                                                    \
		fprintf(stderr, "%*s", (int)(stacktop * 4), ""); \
		fprintf(stderr, __VA_ARGS__);                    \
	} while (0)
#else
#define assert(...)
	...
	while (*s)
	{
		...
		else if (0 == strcasecmp(op, "rotate"))
		{
			...
			if (1 == a)
			{
				...
				r.b = sin(phi);
				r.c = -r.b;
				if (2 == sscanf(s, " , %lf, %lf%n", &m.e, &m.f, &n) || 2 == sscanf(s, " %lf %lf%n", &m.e, &m.f, &n))
				{
					s += n;
...
static int ass_arc(ctx_t *ctx, vec_t v0, vec_t r, double phi, int fa, int fs, vec_t v)
{
	...
	f = sqrt(fabs((r.x * r.x * r.y * r.y - r.x * r.x * p.y * p.y - r.y * r.y * p.x * p.x) / (r.x * r.x * p.y * p.y + r.y * r.y * p.x * p.x)));
...
static int ass_polyline(ctx_t *ctx, const char *pt)
{
	...
	if (pt && *pt && sscanf(pt, "%f ,%f %n", &dummy, &dummy, &n) == 2 && NULL != (d = malloc(strlen(pt) + 4 + 1)))
	{
		strcpy(d, "M ");
		strncat(d, pt, n);
		strcat(d, "L ");
 ...
 */
static void svg2ass(void*arg)
// static void svg2ass(Queue* Q, nxmlEvent_t evt, const nxmlNode_t *node, void *usr)
{
	Queue*Q=(Queue*)arg;
	threadCtx p=de_queue(Q);
	while(p!=NULL)
	{
		nxmlEvent_t evt=p.evt;
		const nxmlNode_t *node=p.node;
		void *usr=p.user;
		int res = 0;
		ctx_t *ctx = usr;
		vec_t v1, v2, c, r;

		if (NXML_TYPE_PARENT != node->type && NXML_TYPE_SELF != node->type && NXML_TYPE_END != node->type)
			return 0;
		...
		case NXML_EVT_OPEN:
			...
			else if (0 == strcasecmp(node->name, "polyline") || 0 == strcasecmp(node->name, "polygon"))
			{
				parseStyles(ctx, node);
				parseTransform(ctx, getStringAttr(node, "transform"));
				ass_line(ctx, ASS_START);
...
static int usage(const char *progname, int version_only)
{
	...
	fprintf(stderr,
			...
			DFLT_EPSILON, MAX_FPREC, DFLT_ARCLINE);
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	while (-1 != (opt = getopt(argc, argv, ostr)))
	{
		...
		case 1: // File ()
			...
			++nfiles;
			break;
		case 'a': //ASS Drawing mode, 1 for multi-shape, 0 for single shape (Default: 1)
