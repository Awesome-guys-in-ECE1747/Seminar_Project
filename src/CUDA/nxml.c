...
__device__ threadCtx de_queue(void *arg){
	Queue *Q = (Queue *)arg;
...
// threadCtx nxmlBegin(threadCtx ctx){
// 	return ctx;
// }

// threadCtx nxmlEnd(threadCtx ctx){
// 	return ctx;
// }
...
int nxmlParse(char *buf, nxmlCb_t cb, void *usr)
{
	...
	Queue *d_Q;
	int res
	cudaMalloc((void**)&d_Q,size*sizeof(Queue));
	cudaMalloc((void**)&d_res,sizeof(int));
	cudaMemcpy(d_Q,&Q,size*sizeof(QNode),cudaMemcpyHostToDevice);
	svg2ass<<<4,4>>>(d_Q,d_res);
